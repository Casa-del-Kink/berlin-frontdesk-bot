...
export function voiceAgentContractChecks(): VoiceAgentContractCheck[] {
  ...
  return [
    ...
    },
    {
      ...
      next: "Run npm run voice:wire-agent from the environment where ELEVENLABS_API_KEY lives, or wire the agent manually in the ElevenLabs console.",
    },
    {
      name: "transcript and recording minimization",
      ok: process.env.VOICE_TRANSCRIPT_STORAGE_REVIEWED === "true" || (!hasEnv("VOICE_STORE_TRANSCRIPT_URLS") && !hasEnv("VOICE_STORE_RECORDING_URLS")),
      severity: "warning",
      ...
      next: "Keep transcript/recording URL storage off or summary-only until Michael/Roxu approve the compliance scope with counsel review as needed.",
    },
    {
...
export function buildVoiceAgentContractReport(): VoiceAgentContractReport {
  ...
  return {
    ...
    localProofBeforeProvider: ["npm run voice:contract:smoke", "npm run voice:post-call:smoke", "npm run voice:smoke", "npm run server:battletest"],
