...
function proofItems(): ProviderProof[] {
  ...
  return [
    ...
    },
    {
      ...
      requiredEnv: ["CALCOM_API_KEY", "one complete Cal.com event type selector", "CALCOM_TEST_ATTENDEE_EMAIL", "CALCOM_SMOKE_APPROVED=true for the smoke run", "CALCOM_SMOKE_TESTED_AT after success"],
    ...
    },
    {
      ...
      requiredEnv: ["OPENROUTER_API_KEY", "LLM_PROVIDER_SMOKE_APPROVED=true"],
      command: "npm run llm:provider:smoke",
      expectedMarker: "LLM_PROVIDER_SMOKE_OK",
      sideEffect: "none",
      approvalRequired: true,
      cleanupProof: "No customer data is sent; the prompt is a fixed German salon connectivity fixture and no app data is created.",
      blockerIfMissing: "Local fake-provider and tool smokes pass, but live WhatsApp language handling is not proven.",
      notes: ["The smoke fails closed unless LLM_PROVIDER_SMOKE_APPROVED=true is set.", "Regression-test the no-secret fail-closed branch with npm run llm:provider:smoke:contract."],
    },
    {
