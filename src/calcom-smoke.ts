...
async function main() {
  assertApproved();
  // This is the only place CALCOM_SMOKE_MODE is set. src/scheduling.ts only honors
  // CALCOM_TEST_ATTENDEE_EMAIL for a live booking's attendee address when this flag is true, so a
  // leftover CALCOM_TEST_ATTENDEE_EMAIL in a real runtime cannot redirect a real customer booking.
  process.env.CALCOM_SMOKE_MODE = "true";
